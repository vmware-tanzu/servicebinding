...
import (
	...
	servicebindinginternalv1alpha2 "github.com/vmware-labs/service-bindings/pkg/client/listers/servicebindinginternal/v1alpha2"
	zap "go.uber.org/zap"
...
// controller reconciling v1alpha2.ServiceBindingProjection.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha2.ServiceBindingProjection. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha2.ServiceBindingProjection) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha2.ServiceBindingProjection.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha2.ServiceBindingProjection. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha2.ServiceBindingProjection) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha2.ServiceBindingProjection if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha2.ServiceBindingProjection.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha2.ServiceBindingProjection) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha2.ServiceBindingProjection if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1alpha2.ServiceBindingProjection.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1alpha2.ServiceBindingProjection) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha2.ServiceBindingProjection) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha2.ServiceBindingProjection resources.
type reconcilerImpl struct {
	...
	Lister servicebindinginternalv1alpha2.ServiceBindingProjectionLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister servicebindinginternalv1alpha2.ServiceBindingProjectionLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.ServiceBindingProjections(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha2.ServiceBindingProjection, desired *v1alpha2.ServiceBindingProjection) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.InternalV1alpha2().ServiceBindingProjections(desired.Namespace)
		...
		updater := r.Client.InternalV1alpha2().ServiceBindingProjections(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha2.ServiceBindingProjection) (*v1alpha2.ServiceBindingProjection, error) {

	getter := r.Lister.ServiceBindingProjections(resource.Namespace)
	...
	patcher := r.Client.InternalV1alpha2().ServiceBindingProjections(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha2.ServiceBindingProjection) (*v1alpha2.ServiceBindingProjection, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha2.ServiceBindingProjection, reconcileEvent reconciler.Event) (*v1alpha2.ServiceBindingProjection, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
